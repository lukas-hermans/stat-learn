...
for (m in m_list) {
  ...
  # find best K value
  K_max = which.max(acc_K) + min(K_list)
  acc_knn = c(acc_knn, acc_K[K_max - min(K_list)])
...
ggplot(dat, mapping = aes(x = m_list, y = value*100, colour = variable)) + geom_line() +
       ...
       scale_x_continuous(breaks = seq(min(m_list), max(m_list), by = 20))

# find best m
m_max = which.max(acc_major_test) + min(m_list)
print(m_max)
...
# find best m
m_max = which.max(acc_log_test) + min(m_list)
print(m_max)
